...
export default function AdminBlogForm() {
    ...
    useEffect(() => {
        ...
        const unsub = onSnapshot(
            ...
            (err) => {
                console.error("Blogs subscribe error:", err);
            }
        );
        return () => unsub();
    }, []);
    ...
    const handleUpload = async (e) => {
        ...
        try {
            const thumbRef = storageRefFn(
                storage,
                `blogs/thumbnails/${Date.now()}-${thumbnailFile.name}`
            );
            ...
            for (let i = 0; i < galleryFiles.length; i++) {
                ...
                const gRef = storageRefFn(
                    storage,
                    `blogs/gallery/${Date.now()}-${i}-${f.name}`
                );
            ...
            const thumbInput = document.getElementById("admin-blog-thumb-input");
            if (thumbInput) thumbInput.value = "";
            const galInput = document.getElementById("admin-blog-gal-input");
            if (galInput) galInput.value = "";
        } catch (error) {
            console.error("Blog upload error:", error);
            if (error.code === 'permission-denied') {
                setMessage("❌ Permission Denied: Please check Firebase Security Rules for the 'blogs' collection and storage path.");
            } else {
                setMessage(`❌ Failed to upload blog: ${error.message || "Unknown error"}`);
            }
        } finally {
            setLoading(false);
        }
        ...
        try {
            if (blog.thumbnailUrl) {
                const thumbPath = decodeURIComponent(
                    blog.thumbnailUrl.split("/o/")[1].split("?")[0]
                );
    ...
    return (
        <div className="flex flex-col items-center pt-6 sm:pt-8 md:pt-12 px-3 sm:px-4 md:px-6">
            ...
            <div className="w-full max-w-4xl px-3 sm:px-4 md:px-0">
                <h2 className="text-lg sm:text-xl font-semibold mb-3 sm:mb-4">Your Blogs</h2>
                    ...
                    {blogs.map((b) => (
                        ...
                        >
                            <div className="flex flex-col">
                                <span className="font-semibold text-sm sm:text-base">{b.title}</span>
                                <span className="text-xs sm:text-sm text-gray-600">
                                    {b.author} {b.place ? `• ${b.place}` : ""}
                                </span>
                            </div>

                            <div className="flex gap-4">
                                <Link to={`/admin/edit-blog/${b.id}`} className="text-purple-600 hover:text-purple-800 text-xs sm:text-sm font-medium">
                                    ✏️ Edit
                                </Link>
                                <button
            ...
            {confirmDelete && (
                <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-white p-4 sm:p-6 rounded-xl w-full max-w-sm sm:w-80 shadow-xl text-center">
                        <h3 className="text-base sm:text-lg font-semibold mb-2 sm:mb-3">Delete Blog?</h3>
